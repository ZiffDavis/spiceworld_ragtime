...
def build_docs(**kwargs):
    ...
    if not config_file and not file and not folder:
        ...
        if config_file.find(".json") == -1:
            click.echo("Config files must be in JSON format.")
            return
        try:
        ...
        if "content_settings" not in config:
            click.echo('No "content_settings" key found in config file.')
            return
        if "document_file" not in config["content_settings"]:
            click.echo('No "document_file" key found in config file.')
            return
        ...
        for key in doc_json:
            ...
            if len(doctext) <= 255:
                ...
                content = unidecode(content, errors="replace", replace_str=" ")
...
def index_docs(**kwargs):
    ...
    if "content_settings" not in config:
        ...
        return
    if "document_file" not in config["content_settings"]:
        click.echo('No "document_file" key found in "content_settings.')
        return
    doc_file = config["content_settings"]["document_file"]
    ...
    if "text_has_labels" in config["content_settings"] and config["content_settings"]["text_has_labels"]:
        if "label_order" not in config["content_settings"]:
            click.echo('No "label_order" key found in "content_settings".')
            return
        if "editor_fields" not in config["content_settings"]:
            click.echo('No "editor_fields" key found in "content_settings".')
            return
