...
class TafiVectorStore:
    ...
    def add_to_index(self, docs=None, index=None, index_name=None, with_llm=False):
        if index is None and index_name is not None:
            index = self.load_index(index_name, with_llm)
        if index is None and index_name is None:
    ...
    def index_from_docs(self, docs=None, index_name=None, with_llm=False):
        storage_context = StorageContext.from_defaults(vector_store=self.get_vector_store(index_name=index_name))
        service_context = self.get_service_context(with_llm)
    ...
    def get_service_context(self, with_llm):
        ...
        service_context = ServiceContext.from_defaults(embed_model=embed_model, llm=llm, node_parser=node_parser)
...
class TafiSimpleVectorStore(TafiVectorStore):
    ...
    def index_from_docs(self, docs=None, index_name=None, with_llm=False):
        logger.info("INDEXING FROM DOCS")
...
class TafiRedisVectorStore(TafiVectorStore):
    ...
    def load_index(self, index_name, metadata_fields: Optional[list]=None, with_llm:bool=True):
        return VectorStoreIndex.from_vector_store(
            vector_store=self.get_vector_store(index_name, metadata_fields),
            service_context=self.service_context(with_llm),
        )
