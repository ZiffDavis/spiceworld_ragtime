...
class PGVectorHelper:
    ...
    def load_index(self, index_name, metadata_fields: Optional[List[str]]=None):
        metadata_fields = metadata_fields or ["url"]
        return VectorStoreIndex.from_vector_store(
            vector_store=self.get_vector_store(), service_context=self.get_service_context()
        )
    ...
    def build_index_from_docs(self, docs, table_name):
        storage_context = StorageContext.from_defaults(vector_store=self.get_vector_store(table_name))
        service_context = self.get_service_context()
